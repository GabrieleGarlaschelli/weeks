...
export type GetWithDownloadParams = GetParams & {
  ...
  fileMime?: 'application/pdf' | 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' | 'text/csv',
  fileName?: string
}
...
export abstract class FetchBasedService {
  ...
  private async __getWithDownload(params: GetWithDownloadParams) {
    return await this.fetch(
      ...
      download(blob, `${params.fileName || 'report'}.${params.fileExt || 'pdf'}`, params.fileMime || 'xlsx')
      return blob
    })
  }
